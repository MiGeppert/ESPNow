...
#else
    #error "Kein Sensor-Profil geladen! Pruefe deine platformio.ini."
#endif
...
volatile bool pendingSleepSave = false;
...
void filterSensorRssiCallback(void *buf, wifi_promiscuous_pkt_type_t type) {
    wifi_promiscuous_pkt_t *pkt = (wifi_promiscuous_pkt_t *)buf;
    if (pkt && pkt->rx_ctrl.rssi < 0 && pkt->rx_ctrl.rssi > -120) {
        dynamicRssi = (float)pkt->rx_ctrl.rssi; // Live-Wert unbestechlich im RTC-RAM sichern!
    }
}
...
void OnDataRecv(const uint8_t * mac_addr, const uint8_t *incoming, int len) {
    ...
    Serial.printf("\n[Funk-Inbound] Paket empfangen! Inhalt: '%s' (Signal: %.0f dBm)\n", msg.c_str(), dynamicRssi);
...
//        sendExpressConfirmation(); // Zündet Express-Rückkanal
        ...
        if (parsedVal >= 10 && parsedVal < 86400) {
            ...
            pendingSleepSave = true; // Flag setzen, um die neue Schlafzeit im Flash zu sichern
//            myData.sleep_seconds = sleepTimeSeconds;
    ...
    for (uint8_t c = 0; c < numChannels; c++) {
        ...
        for (int retry = 1; retry <= RETRIES; retry++) {
            ...
            while (millis() - waitStart < 100) { 
                if (ackReceived) {
                    if (pendingSleepSave) {
//                        Serial.println("  -> Ungespeicherte Schlafzeit im Flash sichern...");
                        preferences.end();
                        if (preferences.begin("sensor_cfg", false)) {
                            preferences.putUInt("sleep_sec", sleepTimeSeconds);
                            preferences.end();
                            Serial.println("  -> [NVS] ERFOLG: Neue Schlafzeit dauerhaft im Flash gesichert (in runMeasurementCycle)!");
                        } else {
                            // -> DEBUG-LOG: Sagt dir sofort, ob die Hardware den Zugriff verweigert!
                            Serial.println("  -> [NVS] FEHLER: preferences.begin verweigert den Schreibzugriff im Interrupt!");
                        }

                        pendingSleepSave = false; // Flag wieder zuruecksetzen
                    }
    ...
    void enterDeepSleep() {
        if (pendingSleepSave) {
            Serial.println("  -> Ungespeicherte Schlafzeit im Flash sichern... in enterDeepSleep()");
            preferences.end();
            Serial.printf("  -> Begin");
            if (preferences.begin("sensor_cfg", false)) {
                preferences.putUInt("sleep_sec", sleepTimeSeconds);
                preferences.end();
                Serial.println("  -> [NVS] ERFOLG: Neue Schlafzeit dauerhaft im Flash gesichert!");
            } else {
                // -> DEBUG-LOG: Sagt dir sofort, ob die Hardware den Zugriff verweigert!
                Serial.println("  -> FEHLER: preferences.begin verweigert den Schreibzugriff im Interrupt!");
            }
    ...
    void setup() {
        ...
        #if defined(IS_ULTRASCHALL)
            // Der Ultraschall-Sensor funkt starr mit 9600 Baud
            SensorSerial.begin(9600, SERIAL_8N1, 16, 17);
            Serial.println("[System] UART initialisiert fuer Ultraschall (9600 Baud, Pins 16/17).");
        #elif defined(IS_RADAR)
            // Das Modbus-Radar verlangt knackige 115200 Baud
            SensorSerial.begin(115200, SERIAL_8N1, 16, 17);
            node.begin(1, SensorSerial);
            Serial.println("[System] UART und Modbus initialisiert fuer Radar (115200 Baud, Pins 16/17).");
        #else
            #error "Kein Sensor-Profil geladen! Pruefe deine platformio.ini."
        #endif
        ...
        if (!isFactoryReset) {
            ...
            if (esp_now_init() == ESP_OK) { 
                ...
                esp_wifi_set_promiscuous_rx_cb(filterSensorRssiCallback);
        ...
        while (WiFi.status() != WL_CONNECTED && millis() - wifiTimeout < 10000) { delay(500); Serial.print("."); }

        if (WiFi.status() != WL_CONNECTED) {
            WiFi.disconnect(true, true); 
            delay(500); WiFi.mode(WIFI_AP); 
            String ap_pass = ap_name; while (ap_pass.length() < 8) ap_pass += "1"; 
